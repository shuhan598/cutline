...
- 修改算法公式前，先补充测试样例并说明业务假设。
- 修改数据模型时，优先使用可选字段兼容旧数据。
- 不同功能的变更尽量分开提交，避免把断料、溢满、切回、混料等逻辑混在一次改动中。
- 涉及车间维度的逻辑应统一使用规范化后的 `workshop_code`。
- 服务层映射只做结果组装，不应在映射阶段重新实现算法判断。
- 每次修改后运行 `pytest`，并在提交说明中写清楚影响范围和测试结果。
